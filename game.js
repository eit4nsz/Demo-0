(() => {
  ...
  const GAME_TEXT = Object.freeze({
    ...
    endingTitle: "Fin del viaje",
    endingLine1: "El cohete llegó a la Luna…",
    endingLine2: "pero Tortu Eitan seguía en la Tierra.",
    catFinalLine: "La tortuga nunca llegará a la luna"
  });
  ...
  const CONFIG = Object.freeze({
    ...
    cruiseSpeed: 174,
    routeSpeedBoost: 36,
    arrivalDurations: Object.freeze({ approach: 3.2, landing: 4, taxi: 3.2, disembark: 4.2, transfer: 4.5, launch: 5.5, moon: 2.3, epilogue: 5 }),
    obstacleGap: [300, 820],
  ...
  const lifeCowMarkup = active => `
    <svg class="life-cow ${active ? "is-active" : "is-empty"}" viewBox="0 0 44 30" aria-hidden="true" focusable="false">
      <path class="cow-wing" d="M13 11C8 3 3 4 2 7c4 1 5 4 7 8z"/>
      <path class="cow-wing" d="M31 11c5-8 10-7 11-4-4 1-5 4-7 8z"/>
      <ellipse class="cow-body" cx="22" cy="14" rx="12" ry="8"/>
      <path class="cow-spot" d="M15 9c4-3 7-1 7 2s-5 5-8 2zM25 16c3-3 7-2 8 1-1 3-5 4-8 2z"/>
      <circle class="cow-body" cx="34" cy="14" r="6"/>
      <ellipse class="cow-muzzle" cx="37" cy="17" rx="5" ry="3"/>
      <circle cx="35" cy="12" r="1.2" fill="#111c31"/>
      <path class="cow-heart" d="M22 29C13 23 17 18 22 22c5-4 9 1 0 7z"/>
    </svg>`;
  ...
  class WorldRenderer {
    ...
    renderMission(ctx, view, game) {
      ...
      if (stage === "EPILOGUE") {
        this.drawEarthEpilogue(ctx, w, h, game.phaseTimer / CONFIG.arrivalDurations.epilogue, game.time, game.reducedMotion);
        return;
      }
      if (stage === "MOON") {
    ...
    drawEarthEpilogue(ctx, w, h, phase, time, reducedMotion) {
      const reveal = smoothstep(0, .18, phase);
      const sky = ctx.createLinearGradient(0, 0, 0, h);
      sky.addColorStop(0, "#101f4d");
      sky.addColorStop(.58, "#3e5380");
      sky.addColorStop(1, "#d27b67");
      ctx.fillStyle = sky; ctx.fillRect(0, 0, w, h);

      const moonX = w * .78, moonY = h * .18, moonR = Math.min(w, h) * .09;
      const moonGlow = ctx.createRadialGradient(moonX, moonY, moonR * .3, moonX, moonY, moonR * 2.2);
      moonGlow.addColorStop(0, "rgba(255,246,190,.5)"); moonGlow.addColorStop(1, "rgba(255,246,190,0)");
      ctx.fillStyle = moonGlow; ctx.beginPath(); ctx.arc(moonX, moonY, moonR * 2.2, 0, Math.PI * 2); ctx.fill();
      ctx.fillStyle = "#fff0b6"; ctx.strokeStyle = "#493d63"; ctx.lineWidth = 4;
      ctx.beginPath(); ctx.arc(moonX, moonY, moonR, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "rgba(101,89,113,.18)";
      [[-.28,-.18,.17],[.22,-.3,.12],[.12,.22,.2]].forEach(([ox,oy,r])=>{ctx.beginPath();ctx.arc(moonX+moonR*ox,moonY+moonR*oy,moonR*r,0,Math.PI*2);ctx.fill();});

      ctx.fillStyle = "rgba(244,244,214,.75)";
      for (let i = 0; i < 48; i += 1) {
        const x = ((i * 173 + 41) % 997) / 997 * w;
        const y = ((i * 97 + 13) % 431) / 431 * h * .56;
        const twinkle = reducedMotion ? .72 : .56 + Math.sin(time * 1.2 + i) * .2;
        ctx.globalAlpha = twinkle; ctx.beginPath(); ctx.arc(x, y, 1 + i % 3 * .35, 0, Math.PI * 2); ctx.fill();
      }
      ctx.globalAlpha = 1;

      const ground = h * .72;
      ctx.fillStyle = "#15273a"; ctx.beginPath(); ctx.moveTo(0, ground);
      for (let x = 0; x <= w + 90; x += 90) ctx.lineTo(x, ground - 18 - Math.sin(x * .025) * 17);
      ctx.lineTo(w, h); ctx.lineTo(0, h); ctx.closePath(); ctx.fill();
      ctx.fillStyle = "#22374b"; ctx.fillRect(w * .08, ground - h * .17, w * .29, h * .17);
      ctx.fillStyle = "#0f1d30"; ctx.fillRect(w * .1, ground - h * .13, w * .25, h * .13);
      ctx.fillStyle = "rgba(255,205,103,.8)";
      for (let x = w * .12; x < w * .34; x += 34) ctx.fillRect(x, ground - h * .1, 14, 7);
      ctx.fillStyle = "#1c3045"; ctx.fillRect(w * .43, ground - h * .22, 13, h * .22);
      ctx.fillStyle = "#37526b"; ctx.beginPath(); ctx.roundRect(w * .405, ground - h * .25, w * .06, h * .055, 6); ctx.fill();
      ctx.fillStyle = "rgba(134,226,221,.72)"; ctx.fillRect(w * .417, ground - h * .235, w * .036, 5);
      ctx.fillStyle = "#24364b"; ctx.fillRect(0, ground, w, h - ground);
      ctx.fillStyle = "rgba(255,199,77,.8)";
      for (let x = 20; x < w; x += 86) { ctx.fillRect(x, ground + 22, 8, 4); ctx.fillRect(x, ground + 22, 2, 25); }

      ctx.save(); ctx.globalAlpha = reveal; ctx.translate(w * .5, h * .62);
      ctx.fillStyle = "#ffc955"; ctx.strokeStyle = "#111c31"; ctx.lineWidth = 4;
      ctx.beginPath(); ctx.roundRect(-112, -24, 224, 48, 7); ctx.fill(); ctx.stroke();
      ctx.fillStyle = "#18233b"; ctx.font = `950 ${clamp(w * .014, 13, 21)}px Segoe UI, sans-serif`; ctx.textAlign = "center";
      ctx.fillText("MIENTRAS TANTO · EN LA TIERRA", 0, 7); ctx.restore();
    }
  ...
  class UIManager {
    constructor(root) {
      ...
      this.characterAnnouncement = root.querySelector("#character-announcement");
      this.epilogueAnnouncement = root.querySelector("#epilogue-announcement");
    ...
    update(player, dt, progress = 0) {
      ...
      if (this.renderedLives !== player.lives) {
        this.lives.innerHTML = Array.from({ length: CONFIG.maxLives }, (_, index) => lifeCowMarkup(index < player.lives)).join("");
        this.renderedLives = player.lives;
      }
    ...
    reset() { this.renderedLives = -1; this.characterAnnouncement.textContent = ""; this.epilogueAnnouncement.textContent = ""; this.region.classList.remove("region-label--visible"); this.message.classList.remove("flight-message--visible"); }
  }
  ...
  class Game {
    ...
    start() {
      ...
      if (preview === "ending") {
        ...
        this.player.position.x = CONFIG.finalDistance * .52;
        this.player.position.y = 3500;
        this.camera.x = this.player.position.x;
        this.camera.altitude = this.player.position.y;
        this.progress = .52;
        this.obstacles.items.push(this.obstacles.createItem("balloon", this.player.position.x + 105, this.player.position.y, getDifficulty(this.progress)));
        this.obstacles.nextSpawnX = this.player.position.x + 1900;
        this.ui.announceRegion("Prueba de aviso de impacto");
      ...
      } else if (preview === "epilogue") {
        this.state = "EPILOGUE";
        this.phaseTimer = 0;
        this.progress = 1;
        this.arrivalSceneProgress = 1;
        this.missionSceneProgress = 1;
        this.player.turtleExitProgress = 1;
        this.ui.hideHud();
        this.ui.epilogueAnnouncement.textContent = `La gata dice: ${GAME_TEXT.catFinalLine}.`;
        this.ui.announceRegion("Tierra · Aeropuerto nocturno");
      }
    }
    ...
    reset(playImmediately = false) {
      ...
      this.arrivalFuel = 100; this.approachStartAltitude = 500; this.touchdownDone = false; this.finalShown = false; this.characterCue = null; this.nextLandmark = 1; this.input.clear();
      this.state = playImmediately ? "PLAYING" : "MENU";
      if (playImmediately) { this.ui.enterGame(); this.ui.announceRegion("La ciudad despierta"); }
    }

    isActiveFlightState() {
      return ["PLAYING", "APPROACH", "LANDING", "TAXI", "ARRIVED", "TRANSFER", "LAUNCH", "MOON", "EPILOGUE"].includes(this.state) && !this.finalShown;
    }
    ...
    update(dt) {
      ...
      this.root.dataset.cue = this.characterCue?.kind || "";
      if (this.state === "MENU" || this.state === "PAUSED" || this.finalShown) return;
      if (this.characterCue) {
        this.characterCue.timer = Math.max(0, this.characterCue.timer - dt);
        if (this.characterCue.timer <= 0) this.characterCue = null;
      }
    ...
    handleCollision(item) {
      ...
      this.characterCue = {
        kind: "cat",
        text: "Ten mucho cuidado,|también hay baches en el cielo",
        timer: 3.4,
        duration: 3.4
      };
      this.camera.hit();
      ...
      this.ui.characterAnnouncement.textContent = "La gata dice: Ten mucho cuidado, también hay baches en el cielo.";
    }
    ...
    handleLifePickup(item) {
      ...
      if (!this.characterCue || this.characterCue.kind !== "cat") {
        this.characterCue = { kind: "cow", text: "¡Te amuuuuuuuuu!", timer: 2.35, duration: 2.35 };
      }
      this.ui.characterAnnouncement.textContent = "La vaca alada dice: ¡Te amuuuuuuuuu!";
    }
    ...
    updateMoon(dt) {
      ...
      if (this.phaseTimer >= CONFIG.arrivalDurations.moon) {
        this.state = "EPILOGUE";
        this.phaseTimer = 0;
        this.ui.epilogueAnnouncement.textContent = `La gata dice: ${GAME_TEXT.catFinalLine}.`;
        this.ui.announceRegion("Tierra · Aeropuerto nocturno");
    ...
    updateEpilogue(dt) {
      this.phaseTimer += dt;
      this.missionSceneProgress = 1;
      if (!this.finalShown && this.phaseTimer >= CONFIG.arrivalDurations.epilogue) {
    ...
    render() {
      ...
      if (["TRANSFER", "LAUNCH", "MOON", "EPILOGUE"].includes(this.state)) {
      ...
      this.renderVignette(ctx, w, h);
      ctx.restore();
      this.renderCharacterCue(ctx, w, h);
      this.progress = actualProgress;
    }
    ...
    renderMissionSequence(ctx) {
      ...
      if (this.state === "TRANSFER") {
        ...
        this.drawRocket(ctx,w*.78,h*.71,0,false,false,false);
      } else if (this.state === "LAUNCH") {
        const phase = clamp(this.phaseTimer / CONFIG.arrivalDurations.launch, 0, 1);
        const ascent = smoothstep(.12,.92,phase);
        const rocketY=lerp(h*.69,-h*.2,ascent);
        this.drawRocket(ctx,w*.62,rocketY,phase,true,false,false);
        ...
        const phase = clamp(this.phaseTimer / CONFIG.arrivalDurations.moon,0,1);
        this.drawRocket(ctx,w*.3,h*.7,1,false,true,false);
        if(phase>.36){ctx.save();ctx.globalAlpha=smoothstep(.36,.66,phase);ctx.fillStyle="#ffe7a1";ctx.strokeStyle="#151b34";ctx.lineWidth=5;ctx.font=`950 ${clamp(w*.021,16,28)}px Segoe UI, sans-serif`;ctx.textAlign="center";ctx.strokeText("¿DÓNDE ESTÁ TORTU EITAN?",w*.58,h*.82);ctx.fillText("¿DÓNDE ESTÁ TORTU EITAN?",w*.58,h*.82);ctx.restore();}
        if (!this.reducedMotion && phase > .78) { ctx.fillStyle=`rgba(0,0,0,${smoothstep(.78,1,phase)})`; ctx.fillRect(0,0,w,h); }
      } else if (this.state === "EPILOGUE") {
        const phase = clamp(this.phaseTimer / CONFIG.arrivalDurations.epilogue,0,1);
        const enter = smoothstep(.04,.25,phase);
        const scale = clamp(w/1040,.72,1.12);
        const turtleX = this.reducedMotion ? w*.41 : lerp(w*.25,w*.41,enter);
        const catX = this.reducedMotion ? w*.63 : lerp(w*.77,w*.63,enter);
        this.drawSuitcase(ctx,turtleX-54*scale,h*.73,scale);
        this.drawMissionTurtle(ctx,turtleX,h*.69,scale,phase>.7);
        this.drawCompanionCat(ctx,catX,h*.69,scale,phase);
        if(phase>.25){
          const bubbleAlpha=smoothstep(.25,.38,phase)*smoothstep(1,.88,phase);
          this.drawSpeechBubble(ctx,w*.5,h*.39,Math.min(470,w-28),106,["La tortuga nunca llegará","a la luna"],"right",bubbleAlpha);
        }
        const fade = 1-smoothstep(0,.16,phase);
    ...
    drawRocket(ctx,x,y,phase,launching,landed=false,showPassenger=false){
      ...
      if(showPassenger){ctx.save();ctx.beginPath();ctx.arc(0,-4,9,0,Math.PI*2);ctx.clip();ctx.fillStyle="#8fbb78";ctx.beginPath();ctx.arc(1,-2,5.2,0,Math.PI*2);ctx.fill();ctx.fillStyle="#527b58";ctx.beginPath();ctx.ellipse(-2,3,7,5,-.1,0,Math.PI*2);ctx.fill();ctx.fillStyle="#1b2c2b";ctx.beginPath();ctx.arc(3,-3,1,0,Math.PI*2);ctx.fill();ctx.restore();}
      else{ctx.fillStyle="#13263b";ctx.beginPath();ctx.arc(0,-4,7.8,0,Math.PI*2);ctx.fill();ctx.fillStyle="rgba(131,219,229,.42)";ctx.beginPath();ctx.arc(-2,-6,2.5,0,Math.PI*2);ctx.fill();}
    ...
    drawSuitcase(ctx,x,y,scale=1){
      ctx.save();ctx.translate(x,y);ctx.scale(scale,scale);
      ctx.strokeStyle="#111c31";ctx.lineWidth=4;ctx.fillStyle="#c9684f";
      ctx.beginPath();ctx.roundRect(-24,-26,48,34,7);ctx.fill();ctx.stroke();
      ctx.strokeStyle="#efb955";ctx.lineWidth=3;ctx.beginPath();ctx.roundRect(-10,-36,20,13,5);ctx.stroke();
      ctx.fillStyle="#f3c768";ctx.fillRect(-19,-18,38,5);ctx.fillStyle="#29495d";ctx.beginPath();ctx.arc(-15,12,5,0,Math.PI*2);ctx.arc(15,12,5,0,Math.PI*2);ctx.fill();ctx.restore();
    }

    drawCompanionCat(ctx,x,y,scale=1,phase=0){
      const tailWave=this.reducedMotion?0:Math.sin(this.time*3.4+phase)*8;
      const blink=!this.reducedMotion&&Math.sin(this.time*.92)> .985;
      ctx.save();ctx.translate(x,y);ctx.scale(scale,scale);ctx.lineJoin="round";ctx.lineCap="round";
      ctx.strokeStyle="#0c1020";ctx.lineWidth=8;ctx.beginPath();ctx.moveTo(20,9);ctx.bezierCurveTo(51,18+tailWave,54,-22,35,-35);ctx.stroke();
      ctx.strokeStyle="#3b455d";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(20,9);ctx.bezierCurveTo(51,18+tailWave,54,-22,35,-35);ctx.stroke();
      const body=ctx.createRadialGradient(-10,-24,3,0,-7,37);body.addColorStop(0,"#4d5268");body.addColorStop(.44,"#202536");body.addColorStop(1,"#0f1322");
      ctx.fillStyle=body;ctx.strokeStyle="#0b0f1d";ctx.lineWidth=4;ctx.beginPath();ctx.ellipse(0,0,27,38,-.08,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.fillStyle="#171b2b";ctx.beginPath();ctx.arc(0,-42,27,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.beginPath();ctx.moveTo(-21,-56);ctx.lineTo(-16,-77);ctx.lineTo(-4,-61);ctx.closePath();ctx.fill();ctx.stroke();
      ctx.beginPath();ctx.moveTo(8,-62);ctx.lineTo(20,-78);ctx.lineTo(22,-55);ctx.closePath();ctx.fill();ctx.stroke();
      ctx.fillStyle="#d7a13f";ctx.beginPath();ctx.arc(-14,-25,3.6,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(12,8,3,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.arc(-7,18,2.3,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle="#efbe48";ctx.lineWidth=blink?4:2.5;[-10,10].forEach(ex=>{ctx.beginPath();ctx.moveTo(ex-5,-45);ctx.quadraticCurveTo(ex,-49,ex+5,-45);ctx.stroke();});
      if(!blink){ctx.fillStyle="#101522";[-10,10].forEach(ex=>{ctx.beginPath();ctx.ellipse(ex,-45,1.7,4,0,0,Math.PI*2);ctx.fill();});}
      ctx.fillStyle="#d78878";ctx.beginPath();ctx.moveTo(-3,-36);ctx.lineTo(3,-36);ctx.lineTo(0,-32);ctx.closePath();ctx.fill();
      ctx.strokeStyle="#8be1d5";ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,-18,19,.18,Math.PI-.18);ctx.stroke();
      ctx.fillStyle="#f0c45d";ctx.strokeStyle="#111c31";ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,1,6,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.restore();
    }

    drawWingedCow(ctx,x,y,scale=1,phase=0,carryingHeart=true){
      const flap=this.reducedMotion?-.08:Math.sin(phase*2.6)*.22;
      ctx.save();ctx.translate(x,y);ctx.scale(scale,scale);ctx.lineJoin="round";ctx.lineCap="round";
      ctx.save();ctx.translate(-22,-8);ctx.rotate(-.28-flap);ctx.fillStyle="#dff5ee";ctx.strokeStyle="#111c31";ctx.lineWidth=3.5;ctx.beginPath();ctx.moveTo(0,7);ctx.quadraticCurveTo(-27,-21,-38,-7);ctx.quadraticCurveTo(-24,-1,-17,15);ctx.quadraticCurveTo(-8,3,0,7);ctx.fill();ctx.stroke();ctx.restore();
      ctx.save();ctx.translate(20,-8);ctx.rotate(.28+flap);ctx.fillStyle="#dff5ee";ctx.strokeStyle="#111c31";ctx.lineWidth=3.5;ctx.beginPath();ctx.moveTo(0,7);ctx.quadraticCurveTo(27,-21,38,-7);ctx.quadraticCurveTo(24,-1,17,15);ctx.quadraticCurveTo(8,3,0,7);ctx.fill();ctx.stroke();ctx.restore();
      const body=ctx.createLinearGradient(0,-20,0,20);body.addColorStop(0,"#fff3c9");body.addColorStop(1,"#d8d3b9");ctx.fillStyle=body;ctx.strokeStyle="#111c31";ctx.lineWidth=4;ctx.beginPath();ctx.ellipse(-5,0,31,21,-.06,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.fillStyle="#26354a";ctx.beginPath();ctx.ellipse(-15,-7,9,7,.4,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.ellipse(3,9,8,5,-.3,0,Math.PI*2);ctx.fill();
      ctx.fillStyle="#fff0bd";ctx.strokeStyle="#111c31";ctx.lineWidth=4;ctx.beginPath();ctx.arc(26,-2,17,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.fillStyle="#efb85d";ctx.beginPath();ctx.moveTo(16,-15);ctx.lineTo(12,-26);ctx.lineTo(21,-18);ctx.closePath();ctx.fill();ctx.stroke();ctx.beginPath();ctx.moveTo(34,-15);ctx.lineTo(40,-25);ctx.lineTo(39,-13);ctx.closePath();ctx.fill();ctx.stroke();
      ctx.fillStyle="#f0a497";ctx.beginPath();ctx.ellipse(31,6,15,9,-.05,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#6d3d4a";ctx.beginPath();ctx.arc(27,5,1.7,0,Math.PI*2);ctx.arc(36,5,1.7,0,Math.PI*2);ctx.fill();
      ctx.fillStyle="#111c31";ctx.beginPath();ctx.arc(22,-5,2.2,0,Math.PI*2);ctx.arc(33,-6,2.2,0,Math.PI*2);ctx.fill();ctx.fillStyle="#fff";ctx.beginPath();ctx.arc(21.4,-5.7,.8,0,Math.PI*2);ctx.arc(32.4,-6.7,.8,0,Math.PI*2);ctx.fill();
      ctx.strokeStyle="#111c31";ctx.lineWidth=4;[-18,4].forEach(lx=>{ctx.beginPath();ctx.moveTo(lx,16);ctx.lineTo(lx-2,28);ctx.stroke();});
      if(carryingHeart){ctx.strokeStyle="#f0bd55";ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(-4,18);ctx.lineTo(-4,27);ctx.stroke();ctx.fillStyle="#ff6f61";ctx.strokeStyle="#7c302e";ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(-4,46);ctx.bezierCurveTo(-24,31,-15,20,-4,29);ctx.bezierCurveTo(7,20,16,31,-4,46);ctx.fill();ctx.stroke();}
      ctx.restore();
    }

    drawSpeechBubble(ctx,x,y,width,height,lines,tailSide="right",alpha=1){
      if(alpha<=0)return;ctx.save();ctx.globalAlpha=clamp(alpha,0,1);ctx.translate(x,y);ctx.shadowColor="rgba(7,15,29,.5)";ctx.shadowBlur=18;ctx.shadowOffsetY=8;
      ctx.fillStyle="#fff0bd";ctx.strokeStyle="#111c31";ctx.lineWidth=5;ctx.beginPath();ctx.roundRect(-width/2,-height/2,width,height,16);ctx.fill();ctx.stroke();ctx.shadowColor="transparent";
      const tailX=tailSide==="right"?width*.31:-width*.31;ctx.beginPath();ctx.moveTo(tailX-18,height/2-2);ctx.lineTo(tailX+10,height/2+25);ctx.lineTo(tailX+23,height/2-2);ctx.closePath();ctx.fill();ctx.stroke();
      ctx.fillStyle="#18233b";ctx.font=`950 ${clamp(width*.045,13,20)}px Segoe UI, sans-serif`;ctx.textAlign="center";ctx.textBaseline="middle";
      lines.forEach((line,index)=>ctx.fillText(line,0,(index-(lines.length-1)/2)*27));ctx.restore();
    }

    renderCharacterCue(ctx,w,h){
      const cue=this.characterCue;if(!cue)return;
      const elapsed=cue.duration-cue.timer;
      const alpha=smoothstep(0,.16,elapsed)*smoothstep(0,.18,cue.timer);
      if(alpha<=0)return;
      const compact=w<620;
      const scale=clamp(w/1050,.7,1);
      ctx.save();ctx.globalAlpha=alpha;
      if(cue.kind==="cat"){
        const catX=compact?w*.82:w-92*scale;
        const catY=compact?h*.66:h*.69;
        const slide=this.reducedMotion?0:lerp(16,0,smoothstep(0,.22,elapsed));
        this.drawCompanionCat(ctx,catX+slide,catY,scale,elapsed);
        this.drawSpeechBubble(ctx,compact?w*.47:w-Math.min(310,w*.27),compact?h*.34:h*.48,Math.min(370,w-24),compact?104:96,["Ten mucho cuidado,","también hay baches en el cielo"],"right",1);
      }else{
        const cowX=compact?w*.2:90*scale;
        const cowY=compact?h*.59:h*.63;
        const lift=this.reducedMotion?0:Math.sin(this.time*4)*4;
        this.drawWingedCow(ctx,cowX,cowY+lift,scale*.9,this.time,true);
        this.drawSpeechBubble(ctx,compact?w*.58:Math.min(310,w*.29),compact?h*.34:h*.5,Math.min(300,w-24),82,["¡Te amuuuuuuuuu!"],"left",1);
      }
      ctx.restore();
    }
    ...
    drawLifePickup(ctx, item, x, y) {
      ...
      ctx.save();ctx.translate(x,y);ctx.scale(pulse,pulse);
      const aura=ctx.createRadialGradient(0,0,5,0,0,58);aura.addColorStop(0,"rgba(255,229,139,.58)");aura.addColorStop(.58,"rgba(99,220,212,.2)");aura.addColorStop(1,"rgba(99,220,212,0)");ctx.fillStyle=aura;ctx.beginPath();ctx.arc(0,0,58,0,Math.PI*2);ctx.fill();
      this.drawWingedCow(ctx,-4,-7,.78,item.phase,true);
      ctx.fillStyle="#fff0bd";ctx.strokeStyle="#111c31";ctx.lineWidth=3;ctx.beginPath();ctx.roundRect(-31,34,62,20,6);ctx.fill();ctx.stroke();ctx.fillStyle="#18233b";ctx.font="950 10px Segoe UI, sans-serif";ctx.textAlign="center";ctx.fillText("+1 VIDA",0,48);
      ctx.restore();
    }
