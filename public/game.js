(() => {
  ...
  class Player {
    ...
    update(dt, input, progress, controlsEnabled = true) {
      ...
      this.velocity.y += (verticalInput * 138 + gentleLift) * dt;
      this.velocity.y *= Math.pow(.966, dt * 60);
      this.velocity.y = clamp(this.velocity.y, -105, 135);
      const desiredSpeed = CONFIG.cruiseSpeed + progress * 30 + horizontalInput * 52;
      this.velocity.x = lerp(this.velocity.x, desiredSpeed, 1 - Math.pow(.94, dt * 60));
      this.velocity.x = clamp(this.velocity.x, 78, 225);
  ...
  class Camera {
    constructor() { this.reset(); }
    reset() { this.x = 0; this.altitude = 190; this.shake = 0; }
    update(dt, player) {
      this.x = lerp(this.x, player.position.x, 1 - Math.pow(.965, dt * 60));
      this.altitude = lerp(this.altitude, player.position.y + player.velocity.y * .85, 1 - Math.pow(.955, dt * 60));
  ...
  class PlaneRenderer {
    render(ctx, x, y, player, time, ending, reducedMotion) {
      const flicker = ending && Math.sin(time * 13) > .25;
      const hitFlash = player.hitReaction > 0 && Math.floor(player.hitReaction * 16) % 2 === 0;
      ctx.save(); ctx.translate(x, y); ctx.rotate(player.rotation);
      ...
      ctx.fillStyle = hitFlash ? "#fff" : "#d9e2e8";
      ctx.beginPath(); ctx.moveTo(-62,-4); ctx.quadraticCurveTo(-32,-25,38,-18); ctx.quadraticCurveTo(76,-12,82,0); ctx.quadraticCurveTo(48,18,-38,17); ctx.lineTo(-64,7); ctx.closePath(); ctx.fill();
      ctx.shadowColor = "transparent";
      ctx.fillStyle = "#9faebd"; ctx.beginPath(); ctx.moveTo(-20,8); ctx.lineTo(28,42); ctx.lineTo(45,40); ctx.lineTo(20,6); ctx.fill();
      ctx.fillStyle = "#c8d6df"; ctx.beginPath(); ctx.moveTo(-46,-7); ctx.lineTo(-60,-41); ctx.lineTo(-42,-40); ctx.lineTo(-23,-2); ctx.fill();
      ctx.fillStyle = "#4b7591"; ctx.beginPath(); ctx.ellipse(16,-17,25,17,0,Math.PI,Math.PI*2); ctx.fill();
      this.drawTurtle(ctx, 15, -25, player, time, ending);
      ctx.fillStyle = "#c76b67"; ctx.beginPath(); ctx.ellipse(67,0,17,12,0,0,Math.PI*2); ctx.fill();
      ctx.strokeStyle = `rgba(228,240,255,${.35 + Math.sin(time*24)*.18})`; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.moveTo(81,-28); ctx.lineTo(81,28); ctx.stroke();
      ctx.fillStyle = flicker ? "rgba(255,190,112,.2)" : "#ffcf73"; ctx.beginPath(); ctx.arc(51,-8,3.5,0,Math.PI*2); ctx.fill();
      ctx.fillStyle = "#74e7cf"; ctx.beginPath(); ctx.arc(-40,7,3,0,Math.PI*2); ctx.fill();
      if (ending) {
        for (let i=0;i<3;i+=1) { const drift=(time*22+i*17)%55; ctx.fillStyle=`rgba(125,137,154,${.25*(1-drift/55)})`; ctx.beginPath(); ctx.arc(-62-drift,-3-Math.sin(time*2+i)*8,5+i*2,0,Math.PI*2); ctx.fill(); }
      }
      ctx.restore();
    }

    drawTurtle(ctx, x, y, player, time, ending) {
      const bob = Math.sin(time * 2.1) * 1.4;
      ctx.save(); ctx.translate(x + (ending ? 3 : 0), y + bob);
      ctx.fillStyle = "#6d9f6f"; ctx.beginPath(); ctx.ellipse(-7,9,17,13,-.1,0,Math.PI*2); ctx.fill();
      ctx.fillStyle = "#91bd78"; ctx.beginPath(); ctx.arc(9,0,11,0,Math.PI*2); ctx.fill();
      ctx.fillStyle = "#2d3a36";
      const eyeY = player.blink > 0 ? 1 : -1;
      if (player.blink > 0) { ctx.fillRect(12,eyeY,5,1.5); }
      else { ctx.beginPath(); ctx.arc(14,eyeY,1.8,0,Math.PI*2); ctx.fill(); }
      ctx.strokeStyle = "#3e654a"; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.arc(12,3,5,.25,1.4); ctx.stroke();
      ctx.strokeStyle = "rgba(42,61,46,.45)"; ctx.beginPath(); ctx.arc(-7,9,10,.5,5.8); ctx.stroke();
      ctx.restore();
    }
  }
  ...
  class Game {
    ...
    update(dt) {
      ...
      if (this.state === "PLAYING") {
        this.progress = clamp(this.player.position.x / CONFIG.finalDistance, 0, 1);
        this.player.update(dt, this.input.keys, this.progress, true);
    ...
    getPlayerScreenPosition() {
      return {
        x: this.view.width * .4,
        y: this.view.height * .53 - (this.player.position.y - this.camera.altitude) * .28
      };
    }
    ...
    render() {
      ...
      this.planeRenderer.render(ctx, point.x, point.y, this.player, this.time, this.state === "ENDING" || this.state === "GAME_OVER", this.reducedMotion);
    ...
    renderObstacles(ctx) {
      ...
      for (const item of this.obstacles.items) {
        ...
        const y = this.view.height * .53 - (item.y - this.camera.altitude) * .28;
